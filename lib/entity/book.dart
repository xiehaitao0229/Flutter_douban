...
class Book {
  ...
  Book(this.name, this.imageAddress, this.author, this.ratingsValue,
      this.address);
}
...
class BookTitleList {
  ...
  factory BookTitleList.format(String htmlDoc, String id) {
    ...
    List<Element> c =
        a.getElementsByClassName('bd').first.getElementsByTagName('li');
    List<Book> d = [];
    c.forEach((e) {
      ...
      if (star.length != 0) {
        String value = star.first
            .getElementsByClassName('average-rating')
            .first
            .text
            .replaceAll(" ", '');
